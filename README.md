...
Учебный проект по сегментации дефектов на изображениях деталей конвейера.  
Проект специально упрощён для сдачи преподавателю: одна модель, понятный пайплайн, минимум лишних зависимостей и автоматическое сохранение наглядных результатов.
...
## Почему структура упрощена

В проекте удалены лишние файлы, связанные с:

- автоэнкодером и anomaly detection;
- лишними метриками и экспериментальными скриптами;
- старыми графиками и промежуточными артефактами.

Итог: проект сфокусирован только на одной задаче, которую легко объяснить на защите.
...
Это удобно для демонстрации сегментации и корректно объясняется на защите как учебное решение.
...
Для показа преподавателю проект оформлен вокруг одной итоговой задачи ClearML и набора локальных материалов:
...
## Что говорить про валидационную ошибку
...
## Как объяснить результат преподавателю
